...
@section('content')
                    ...
                    <div class="table-responsive">
                        <table id="usuarios-conectados-table" class="table table-bordered table-striped table-hover">
                            <thead class="thead-light">
                                <tr>
                                    <th class="text-center" title="Usuario"><i class="fas fa-user"></i></th>
                                    <th class="text-center" title="Email"><i class="fas fa-envelope"></i></th>
                                    <th class="text-center" title="Rol"><i class="fas fa-user-tag"></i></th>
                                    <th class="text-center" title="Nodo"><i class="fas fa-sitemap"></i></th>
                                    <th class="text-center" title="Socio"><i class="fas fa-handshake"></i></th>
                                    <th class="text-center" title="Última Actividad"><i class="fas fa-clock"></i></th>
                                    <th class="text-center" title="Tiempo Inactivo"><i class="fas fa-hourglass-half"></i></th>
                                    <th class="text-center" title="Dirección IP"><i class="fas fa-map-marker-alt"></i></th>
                                    <th class="text-center" title="Navegador"><i class="bi bi-globe2"></i></th>
                                    <th class="text-center" title="Acciones"><i class="fas fa-cogs"></i></th>
                                </tr>
                            </thead>
                            <tbody>
                                @forelse($usuariosConectados as $usuario)
                                    <tr>
                                        ...
                                        <td title="{{ $usuario->nodo_nombre ?? 'N/A' }}">
                                        ...
                                        <td title="{{ $usuario->socio_nombre ?? 'N/A' }}">
                                        ...
                                        <td title="{{ $usuario->last_activity_human }}">
                                            <small class="text-muted">{{ $usuario->last_activity_human }}</small>
                                        </td>
                                        <td title="Inactivo por {{ $usuario->tiempo_inactivo }} minutos">
                                            @php
                                                $badgeClass = 'success';
                                                if ($usuario->tiempo_inactivo > 60) {
                                                    $badgeClass = 'danger';
                                                } elseif ($usuario->tiempo_inactivo > 30) {
                                                    $badgeClass = 'warning';
                                                }
                                            @endphp
                                        ...
                                        <td title="{{ $usuario->ip_address }}">
                                            <code class="text-sm">{{ $usuario->ip_address }}</code>
                                        </td>
                                        <td title="{{ $usuario->user_agent ?? 'No disponible' }}">
                                            @php
                                                $browser = '';
    ...
    <script>
        ...
        function desconectarTodos() {
            if (!confirm(
                    '¿Está seguro de desconectar a TODOS los usuarios conectados?\n\nEsta acción no se puede deshacer.')) {
                return;
            }
...
@section('css')
    ...
    <style>
        ...
        /* Forzar el respeto de anchos de columnas */
        table.dataTable {
    ...
    <script>
        ...
        function addCustomFilters() {
            if (typeof $ !== 'undefined' && $('#usuarios-conectados-table_wrapper').length) {
                ...
                <
                label
                for = "filter-tiempo" > Filtrar por actividad: < /label> <
                ...
                <
                label
                for = "filter-nodo" > Filtrar por nodo: < /label> <
                select id = "filter-nodo"
                class = "form-control form-control-sm" >
                <
                ...
                <
                label
                for = "filter-rol" > Filtrar por rol: < /label> <
